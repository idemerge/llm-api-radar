...
export const workflowTemplates: WorkflowTemplate[] = [
  ...
  // ────────────────────────────────────────────────
  {
    ...
    description: 'Single-task benchmark — pick providers, set a prompt, and run. The fastest way to compare LLM performance.',
    tasks: [
      {
        name: 'Benchmark',
        config: {
  ...
  // ────────────────────────────────────────────────
  {
    ...
    tasks: [
      ...
      { name: '50 Concurrent', description: 'Stress — find the breaking point', config: { prompt: 'Write a Python function that performs binary search on a sorted list.', concurrency: 50, iterations: 20, maxTokens: 200, streaming: true }, tags: { concurrency: '50' } },
    ],
  ...
  // ────────────────────────────────────────────────
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        config: { prompt: 'Write a production-ready Go HTTP middleware that implements rate limiting using a sliding window counter algorithm with Redis as the backing store. Include proper error handling, context cancellation support, and configuration.', maxTokens: 1000, concurrency: 3, iterations: 10, streaming: true },
        tags: { category: 'code' },
      },
      {
        name: 'Creative Writing',
        description: 'Narrative coherence and style',
        config: { prompt: 'Write a 400-word flash fiction story about a debugger that discovers its own source code contains a message from its creator. The tone should shift from technical to philosophical.', maxTokens: 600, concurrency: 3, iterations: 10, streaming: true },
        tags: { category: 'creative' },
      },
      {
        name: 'Technical Analysis',
        description: 'Structured reasoning and depth',
        config: { prompt: 'Analyze the trade-offs between using gRPC vs REST for microservice communication. Consider performance, developer experience, ecosystem support, streaming capabilities, and operational complexity. Provide a recommendation framework.', maxTokens: 800, concurrency: 3, iterations: 10, streaming: true },
        tags: { category: 'analysis' },
      },
      {
        name: 'Long Context Reasoning',
        description: 'Information synthesis over large inputs',
        config: { prompt: 'Review the following three database migration strategies: 1) Big bang migration, 2) Dual-write pattern, 3) Event-driven migration. For each strategy, describe the implementation approach, rollback plan, data consistency guarantees, and estimated downtime. Recommend which to use for migrating a 50TB PostgreSQL database to a new schema with zero downtime.', maxTokens: 1500, concurrency: 2, iterations: 5, streaming: true },
      ...
      },
      {
        ...
        config: {
          prompt: 'Generate a JSON object representing a CI/CD pipeline configuration with the following fields: name (string), stages (array of objects with name, steps, and timeout), triggers (array of branch patterns), and notifications (object with slack and email configs). Output valid JSON only.',
          systemPrompt: 'You are a DevOps configuration generator. Always respond with valid JSON. No markdown fences, no explanations — pure JSON output only.',
          maxTokens: 500, concurrency: 3, iterations: 10, streaming: true,
        },
        tags: { category: 'json' },
      },
    ],
  ...
  // ────────────────────────────────────────────────
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        config: { prompt: 'Explain how Kubernetes handles pod scheduling including node affinity, taints/tolerations, and resource limits. Provide 3 paragraphs.', concurrency: 5, iterations: 20, maxTokens: 300, streaming: true },
        tags: { cost_tier: 'standard' },
      },
      {
        name: 'Heavy Tier (2000t)',
        description: 'Large outputs — measure cost efficiency at scale',
        config: { prompt: 'Write a complete Terraform module for deploying a production-grade AWS EKS cluster with VPC, subnets, node groups, IAM roles, security groups, and a Helm release for monitoring. Include all necessary variables, outputs, and comments explaining each resource.', concurrency: 2, iterations: 10, maxTokens: 2000, streaming: true },
  ...
  // ────────────────────────────────────────────────
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        config: { prompt: 'Generate a unique 16-character alphanumeric session token and return it as JSON: {"token": "..."}', concurrency: 25, iterations: 40, maxTokens: 50, streaming: false },
        tags: { phase: 'heavy' },
      },
      {
        name: 'Burst Stress (50c × 30i)',
        description: 'Maximum load to measure degradation and recovery',
        config: { prompt: 'Return the current response code 200 as JSON.', concurrency: 50, iterations: 30, maxTokens: 20, streaming: false, requestInterval: 50 },
        tags: { phase: 'burst' },
      },
    ],
    options: { stopOnFailure: false, cooldownBetweenTasks: 5000 },
  ...
  // ────────────────────────────────────────────────
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        config: { prompt: 'Review this function for bugs, performance issues, and style: def get_user(id): user = db.query("SELECT * FROM users WHERE id=" + id); return user', maxTokens: 500, concurrency: 5, iterations: 15, streaming: true, requestInterval: 500, randomizeInterval: true },
        tags: { type: 'code-review', behavior: 'moderate' },
      },
      {
        name: 'Document Generation',
        description: 'Long-form output simulating report generation',
        config: { prompt: 'Generate a project post-mortem report for a failed database migration. Include executive summary, timeline, root cause analysis, lessons learned, and action items.', maxTokens: 1500, concurrency: 2, iterations: 8, streaming: true, requestInterval: 1000, randomizeInterval: true },
      ...
      },
      {
        ...
        config: { prompt: 'Generate an OpenAPI 3.0 specification for a blog platform with endpoints for posts, comments, authors, and categories. Include request/response schemas and error responses.', maxTokens: 1000, concurrency: 3, iterations: 10, streaming: true, requestInterval: 800, randomizeInterval: true },
  ...
  // ────────────────────────────────────────────────
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        config: {
          prompt: 'Find all users who signed up in the last 30 days and have placed at least 2 orders with a total value over $500.',
          systemPrompt: 'You are a SQL query builder. Convert the user request into a function call: execute_query(sql: string, database: string). Respond with JSON: {"function": "execute_query", "arguments": {"sql": "...", "database": "..."}}. No other text.',
          maxTokens: 300, concurrency: 3, iterations: 15, streaming: true,
        },
      ...
      },
      {
        ...
        config: {
          prompt: 'Parse this email: "From: john.doe@acme.com\nTo: support@saas.io\nSubject: Urgent: Server down since 3am\nHi team, Production server web-03 has been unresponsive since 3:00 AM EST. CPU was at 98% before it stopped responding. Please escalate immediately. — John, DevOps Lead"',
          systemPrompt: 'Extract structured data from emails. Always respond with valid JSON: {"sender": {"email": "", "name": ""}, "recipient": {"email": ""}, "subject": "", "priority": "low"|"medium"|"high"|"critical", "category": "", "summary": "", "action_items": [], "mentioned_systems": []}. No markdown, no explanations.',
          maxTokens: 300, concurrency: 3, iterations: 15, streaming: true,
        },
      ...
      },
      {
        ...
        config: {
          prompt: 'Create a product listing API response that includes 2 products, each with id, name, price, inventory count, categories (array), and a nested availability object with inStock boolean and restockDate.',
          systemPrompt: 'You are an API mock data generator. Always respond with valid JSON matching the requested schema. No markdown fences, no commentary — pure JSON only.',
          maxTokens: 400, concurrency: 3, iterations: 15, streaming: true,
        },
  ...
  // ────────────────────────────────────────────────
  {
    ...
    description: 'Test vision/image understanding capabilities — models receive a screenshot and must describe its content',
    tasks: [
      {
        name: 'Image Description',
        description: 'Describe the contents of the provided screenshot',
        config: {
          prompt: 'Describe the user interface shown in this screenshot in detail. What are the main sections, components, and features visible?',
          maxTokens: 500,
          concurrency: 3,
          iterations: 10,
          streaming: true,
      ...
      },
      {
        ...
        config: {
          prompt: 'Extract all visible text and labels from this screenshot. List them in the order they appear, organized by section.',
          maxTokens: 300,
          concurrency: 3,
          iterations: 10,
          streaming: true,
      ...
      },
      {
        ...
        config: {
          prompt: 'Analyze the UI/UX design of this application screenshot. Comment on the layout structure, color scheme, navigation pattern, and overall usability. Provide specific, actionable feedback.',
